...
public class Player1MoveController : MonoBehaviour
{
    ...
    public Vector2 ForceY = new Vector2(0, 150);

    // 速度
    private float horizontalDirection; // 水平方向
    private float speed_X = 5f; // 水平移動的速度
    ...
    void Start()
    {
        ...
        GameObject.Find("Player").GetComponent<DashMove>().enabled = false;
        GameObject.Find("Player").GetComponent<DashMove>().enabled = true;
    ...
    */
    ...
    void Jump_Down_Animator()
    {
        ...
        {
            playerRigidbody2D.AddForce(ForceY, ForceMode2D.Impulse);
